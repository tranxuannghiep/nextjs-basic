...
import { CONFIG, NOT_FOUND_INDEX, formatPrice, QUERY_KEY_PRODUCT } from '@/utils';
...
export default function ProductDetail() {
  ...
  const { data: product, isLoading } = useQuery({
    queryKey: [QUERY_KEY_PRODUCT.GET_DETAIL, productId as string],
    queryFn: productApi.productDetail,
    enabled: !!productId,
    select: (res) => res.data,
  ...
  const handleIncreaseAmount = (product: ProductType) => {
    setAmount((prev) => (prev === product.quantity ? prev : prev + 1));
  };

  const handleClickBuy = (product: ProductType) => {
